...
#         sentences = NPChunkCorpus('C:/Users/Sandro/PycharmProjects/HMM_PA3/resource/np_chunking_wsj', document_class=document_class)
#         seed(hash("np_chunk"))
#         shuffle(sentences)
...
def split_np_chunk_corpus(document_class):
    """Split the yelp review corpus into training, dev, and test sets"""
    sentences = NPChunkCorpus('C:/Users/Sandro/PycharmProjects/HMM_PA3/resource/np_chunking_wsj', document_class=document_class)
    seed(hash("np_chunk"))
    shuffle(sentences)
...
train, test = split_np_chunk_corpus(Document)
classifier = HMM()
classifier.train(train)
test_result = compute_cm(classifier, test)
